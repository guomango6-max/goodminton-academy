...
import { setStoredLang, useLang } from './components/LangContext';
...
export default function Home({
  ...
  /** 服务端读 content/articles 得到的真实文章卡片。 */
  articles: Record<Lang, ArticlePost[]>;
  /** /en 下固定英文，不看本地偏好。 */
  forcedLang?: Lang;
}) {
  const context = useLang();
  const lang = forcedLang ?? context.lang;
  const t = copy[lang];
  ...
  // 语言现在是地址的一部分（/ 与 /en），所以切换要换地址，不能只翻一个本地开关：
  // 英文版必须有自己可被索引、可被分享的 URL。本地偏好照旧写入，好让学员页、
  // 论坛那些仍靠 localStorage 的页面跟着走。
  function toggle() {
  ...
  useEffect(() => {
    ...
    // 文章不再在这里拉：它由服务端随页面一起渲染，HTML 里就是最新的三篇。
    // 以前这里 fetch 一次，只是为了盖掉本文件里那份写死的旧副本——爬虫看不到
    // 这一步，于是索引到的一直是旧副本。
  ...
  async function handleForumStudentLogin(rawCredential: string) {
    ...
    try {
      ...
      const profile = profileResponse?.ok
        ? ((await profileResponse.json().catch(() => ({}))) as { nickname?: string })
        : {};
      window.sessionStorage.setItem('goodminton-forum-identity', JSON.stringify({
        kind: 'student',
        label: profile.nickname || (lang === 'zh' ? '学员' : 'Student'),
      }));
      window.location.assign('/forum');
    } catch (reason) {
      setForumLoginError(reason instanceof Error ? reason.message : (lang === 'zh' ? '登录失败。' : 'Login failed.'));
      setForumLoginBusy(false);
    }
  }
